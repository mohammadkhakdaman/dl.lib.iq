...
export default function AuthorCard({ item }) {
    ...
    return item && <>
        <div class="text-sm leading-6">
            <Link href={`/${language}/books?q=${item.key}&idx=["Subject"]`}>
                <figure class="relative flex flex-col-reverse bg-slate-100 rounded-lg p-6 dark:bg-slate-800 dark:highlight-white/5">
                    <figcaption class="flex items-center space-x-4">
                        <img src="/static/media/subject.png" alt="" class="flex-none me-5 w-14 h-14 rounded-full object-cover" loading="lazy" decoding="async" />
                        <div class="flex-auto">
