...
import { BookResultItemHeader } from "."

export default function BookListItem({ item, injectModal }) {
    ...
    return (
        <div className="p-4 pb-2 bg-gray-100 border rounded relative mx-auto w-full text-sm">
            <BookResultItemHeader book={item} className={''}/>

            <div className="flex items-start mt-6 gap-5 p-2 pb-0">
                <div>
                    <Link href={`/${language}/books/${item.bookId}/1`} target="_blank">
                ...
                <div className="flex gap-2 flex-col flex-1">
                    ...
                    }
                    {
                        item?.pages?.map((page, index) => {
                            ...
                            return (
                                <Link href={`/${language}/books/${item.bookId}/${page.bookPageNumber}`} 
                                      key={page.bookPageNumber} className="p-3 border border-gray-300 bg-white rounded-lg flex gap-2 items-start">
                    ...
                    {
                        item?.pages?.length > 2 &&
                        <button onClick={contetnModal} className="p-2 text-gray-700 hover:scale-105 duration-75 ">
                            مشاهده‌ی نتایج بیشتر...
                        </button>
                    }
                </div>
            </div>
